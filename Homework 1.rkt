...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname |Homework 1|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
(define (play w)
  (big-bang w [to-draw world->image] [on-key key-pressed]))

;; world->image : World -> Image
;; Consumes a world and outputs an image of the world
(define (world->image w)
  (beside (overlay (above (text (world-pw w) 40 "black") (letters->image (world-l w)))
...
(define (letters->image alist)
  (cond
    ...
    [(cons? alist)
     (overlay/offset (overlay (text (first alist) 20 "black") (circle 30 "solid" (color 230 230 230)))
...
(check-expect (letters->image (list "A"))
              (overlay (overlay (text "A" 20 "black") (circle 30 "solid" (color 245 218 73)))
                       (square 350 "solid" "white")))

(check-expect (letters->image (list "A" "B" "C"))
              (overlay/offset
               (overlay (text "A" 20 "black") (circle 30 "solid" (color 230 230 230)))
               (* 65 (cos (* (/ (* 2 pi) 6) 2)))
               (* 65 (sin (* (/ (* 2 pi) 6) 2)))
               (overlay/offset
                (overlay (text "B" 20 "black") (circle 30 "solid" (color 230 230 230)))
                (* 65 (cos (* (/ (* 2 pi) 6) 1)))
                (* 65 (sin (* (/ (* 2 pi) 6) 1)))
                (overlay (overlay (text "C" 20 "black") (circle 30 "solid" (color 245 218 73)))
                         (square 350 "solid" "white")))))
...
(define (prev-list->image prev)
  (cond
    ...
    [(cons? prev) (above (text (first prev) 18 "black") (prev-list->image (rest prev)))]))
